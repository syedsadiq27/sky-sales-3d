...
let strapMat,glassMat,frameMat,timeIndex,dailMat,reverseMetal,strapInnerMat;
...
$(document).ready(function () {
  ...
  strapMat = new THREE.MeshStandardMaterial({
    ...
    reflectivity:.5,
    envMapIntensity:3,
    map: textureLoad("assets/tex/leather/leather.jpg"),
    aoMap : textureLoad("assets/tex/leather/AO.png"),
    aoMapIntensity:.7,
    normalMap: textureLoad("assets/tex/leather/NormalMap.png"),
    normalScale: new THREE.Vector3( 0.1, 0.1),
    roughnessMap : textureLoad("assets/tex/leather/SpecularMap.png"),
    displacementMap :textureLoad("assets/tex/leather/DisplacementMap.png"),
    displacementScale : .003,
    roughness : .7,
    metalness : .5,
  ...
  frameMat = new THREE.MeshStandardMaterial({
    ...
    normalScale: new THREE.Vector3( 0.01, 0.01),
  ...
  strapInnerMat = new THREE.MeshStandardMaterial({
    envMap : envMap,
    reflectivity:.1,
    envMapIntensity:3,
    map: textureLoad("assets/tex/Inner_leather_01.png"),
    // aoMap : textureLoad("assets/tex/leather/AO.png"),
    // aoMapIntensity:.7,
    // normalMap: textureLoad("assets/tex/leather/NormalMap.png"),
  ...
  reverseMetal =  new THREE.MeshStandardMaterial({
    ...
    reflectivity : 1,
    envMapIntensity:.5,
    map: textureLoad("assets/tex/Back_text.png"),
     roughness : .3,
     metalness : .5,
    combine: THREE.MixOperation,
    side: THREE.DoubleSide,
...
class sceneSetup {
  ...
  /*  this.composer = new EffectComposer( this.renderer );
    ...
    this.ssaoPass = new SSAOPass( this.scene, this.cameraMain, false, true );
   
    this.ssaoPass.params = { 
      output: 0, 
      saoIntensity: 0.1, 
      saoScale: 1, 
      saoKernelRadius: 40, 
...
let tgaLoad = (texPath) => {
  const loader = new THREE.TGALoader().load();
}
     ...
     gltf.scene.traverse(function (node) {
        if (node.type === "Mesh") {
          ...
          node.receiveShadow = true;
            ...
            }else if(node.name == 'timeIndex' || node.name == 'policeText'){
           ...
           }else if(node.name == 'StrapStitches'){
            node.material = new THREE.MeshPhongMaterial({
              color:0x1f0f00,
              combine: THREE.MixOperation,
              side: THREE.DoubleSide,
            });
         }else if(node.name == 'strapInner'){
          node.material = strapInnerMat;
       }else if(node.name == 'centerMetalOuter'){
        node.material = new THREE.MeshPhongMaterial({
          map:textureLoad("assets/tex/Watch_bg_1.png"),
          color:0xffe7d1,
          combine: THREE.MixOperation,
          side: THREE.DoubleSide,
        });
